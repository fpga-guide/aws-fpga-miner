...
enum algos {
	...
	ALGO_SKUNK,       /* Skein Cube Fugue Gost */
	ALGO_S3,          /* S3 */
...
int g_miner_count = 0;
int g_fpga_count = 0;
int g_fpga_work_len = 0;
int g_fpga_data_start = 0;
int g_fpga_data_len = 0;
int g_fpga_mid_len = 0;
bool g_fpga_use_target = false;
bool g_fpga_swap_data = false;
int g_fpga_nonce_idx = 19;
...
Options:\n\
  ...
  -f, --diff-factor <n>      Divide target diff by this factor (Default: 1.0)\n\
  -m, --diff-multiplier <n>  Multiply target diff by this factor (Default: 1.0)\n\
...
static char const short_options[] =
	"a:b:c:Df:F:hm:p:Px:qr:R:s:t:T:o:u:O:V";
...
static void calc_network_diff(struct work *work)
{
	...
	for (m = shift; m < 29; m++) d *= 256.0;
	for (m = 29; m < shift; m++) d /= 256.0;
...
static int share_result(int result, struct work *work, const char *reason)
{
	...
	char suppl[32] = { 0 };
	char s[345];
	double hashrate, diff_factor, sharediff;
//	double sharediff = work ? work->sharediff : stratum.sharediff;
	...
	sharediff = stratum_diff / diff_factor;
...
//				le32enc(&nonce, work->data[35]);
    ...
    char votehex[16] = { 0 };
			...
			if (opt_vote) { // ALGO_HEAVY ALGO_DECRED
				bin2hex(nvotestr, (const unsigned char *)(&nvote), 2);
				sprintf(s, "{\"method\": \"mining.submit\", \"params\": ["
						"\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], \"id\":%u}",
						rpc_user, work->job_id, xnonce2str, ntimestr, noncestr, nvotestr, 4);
				free(nvotestr);
			} else {
				sprintf(s, "{\"method\": \"mining.submit\", \"params\": ["
						"\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"], \"id\":%u}",
						rpc_user, work->job_id, xnonce2str, ntimestr, noncestr, 4);
			}
			free(xnonce2str);
		}
...
static bool get_work(struct thr_info *thr, struct work *work)
{
	...
	if (opt_benchmark) {
		...
		for (n = 0; n < 74; n++) ((char*)work->data)[n] = n;
...
//			else
		...
		// Test Data
		//
//		unsigned char test_data[] = { 0x00, 0x00, 0x00, 0x07, 0xa7, 0x29, 0x51, 0xa8, 0xc3, 0xfa, 0x53, 0x6d, 0x55, 0xd3, 0x9e, 0xf4, 0x6a, 0xc3, 0x70, 0xd4, 0x65, 0xa8, 0xd2, 0x5d, 0x6c, 0x97, 0xff, 0x00, 0x33, 0xa1, 0x21, 0xe0, 0x99, 0xfe, 0x92, 0x0e, 0xb6, 0x84, 0x1d, 0x58, 0xd7, 0x16, 0x18, 0x9b, 0x1f, 0xa1, 0xf2, 0x01, 0x55, 0x6a, 0x02, 0x5b, 0x00, 0xf5, 0xab, 0xd3, 0x43, 0x87, 0xd3, 0x1c, 0xfd, 0x98, 0x47, 0x11, 0xbc, 0x2a, 0x41, 0x55, 0x59, 0xba, 0x73, 0x87, 0x1b, 0x08, 0xb7, 0xf4, 0x04, 0x2a, 0x73, 0x00 };
//		memcpy(work->data, test_data, 80);
		//
		// Test Data
		//
...
void parse_arg(int key, char *arg)
{
	...
	case 'F':
		v = atoi(arg);
		if ((v != 1) && (v != 8))	/* AWS Supports 1 or 8 FPGAs */
			show_usage_and_exit(1);
		opt_aws_fpgas = v;
		break;
	case 'm':
...
//
// Begin FPGA Specific Logic
//

static bool initialize_aws_miner(void *thr, int slot_id)
{
...
extern void calc_hash(unsigned char *data, const unsigned char *hash)
{
	...
	switch (opt_algo) {
		...
		case ALGO_VANILLA:
			swap_endian(endian_data, data32, 80);
			blakecoinhash((void *)hash, (void *)endian_data);
			break;
		case ALGO_DECRED:
			decred_hash((void *)hash, (void *)data);  // Decred Data Is Already Swapped
			break;
		case ALGO_PHI:
			swap_endian(endian_data, data32, 80);
			phi_hash((void *)hash, (void *)endian_data);
			break;
		case ALGO_POLY:
			swap_endian(endian_data, data32, 80);
			poly_hash((void *)hash, (void *)endian_data);
			break;
		case ALGO_SKUNK:
			swap_endian(endian_data, data32, 80);
			skunk_hash((void *)hash, (void *)endian_data);
			break;
		case ALGO_TRIBUS:
			swap_endian(endian_data, data32, 80);
			tribus_hash((void *)hash, (void *)endian_data);
			break;
		case ALGO_NIST5:
			swap_endian(endian_data, data32, 80);
...
extern void calc_midstate(unsigned char *data, const unsigned char *midstate)
{
	...
	switch (opt_algo) {
		...
		case ALGO_VANILLA:
			swap_endian(endian_data, data32, 80);
			blakecoinmidstate((void *)midstate, (void *)endian_data);
			break;
		case ALGO_DECRED:
			decred_midstate((void *)midstate, (void *)data);	// Decred Data Is Already Swapped
			break;
		case ALGO_PHI:
			swap_endian(endian_data, data32, 80);
			phi_midstate((void *)midstate, (void *)endian_data);
			break;
		case ALGO_POLY:
			swap_endian(endian_data, data32, 80);
			poly_midstate((void *)midstate, (void *)endian_data);
			break;
		case ALGO_SKUNK:
			swap_endian(endian_data, data32, 80);
			skunk_midstate((void *)midstate, (void *)endian_data);
			break;
		case ALGO_TRIBUS:
			swap_endian(endian_data, data32, 80);
			tribus_midstate((void *)midstate, (void *)endian_data);
			break;
		case ALGO_NIST5:
			swap_endian(endian_data, data32, 80);
			nist5_midstate((void *)midstate, (void *)endian_data);
			break;
		default:
...
static void *aws_miner_thread(void *userdata)
{
	...
	struct work work = {{0}};
	int i, rc, fd, ret;
	unsigned char data[192], midstate[128], send_buf[144], nonce_buf[4];
	uint32_t *target;
	...
	while (1) {
		...
		if ( g_fpga_mid_len ) {
			...
			memcpy(data, midstate, g_fpga_mid_len);	// Copy Midstate To Data Buffer
			memcpy(data + g_fpga_mid_len, (unsigned char*)work.data + g_fpga_data_start, g_fpga_data_len);	// Copy Remaining Bytes Of Block Header
		}
		else {
		...
		if ( g_fpga_use_target ) {
			if ( g_fpga_swap_data )
				...
				uint32_t t = swab32(work.target[7]);
				memcpy(data + g_fpga_mid_len + g_fpga_data_len, (unsigned char*)(&t), 4);  // Pass H7 Target To FPGA
			}
//			memset(data + g_fpga_mid_len + g_fpga_data_len, 0, 4);  // Pass H7 Target To FPGA
		}

		// Change Endianess On Each 4 Byte Chunk
		if ( g_fpga_swap_data ) {
			if (g_fpga_mid_len == 64) {
				// Swap Midstate
				swap_endian_64(send_buf, data, g_fpga_mid_len);
				// Swap remaining data
				swap_endian(send_buf + 64, data + 64, g_fpga_work_len - g_fpga_mid_len);
			}
			else {
			...
			if (g_fpga_mid_len == 128) {
				// Copy Midstate To Buffer
				memcpy(send_buf, data32, 140);
				// Copy Remaining Data To FPGA Message
				b32[32] = swab32(data32[32]);
				b32[33] = swab32(data32[33]);
				b32[34] = swab32(data32[34]);
				b32[35] = data32[35]; // Target
			}
			else if (g_fpga_mid_len == 64) {
				// Swap Midstate
				swap_endian_64(send_buf, data, g_fpga_mid_len);
				// Copy Remaining Data Reversing Order Of 4 Byte Chunks
				b32[16] = data32[18];
				b32[17] = data32[17];
				b32[18] = data32[16];
				b32[19] = data32[19];
				...
				// Swap Midstate
				swap_endian(send_buf, data, g_fpga_mid_len);
				// Copy Remaining Data Unswapped
				memcpy(send_buf + g_fpga_mid_len, data + g_fpga_mid_len, g_fpga_work_len - g_fpga_mid_len);
			}
		}

		// Dump FPGA Data Buffer
		if (opt_debug) {
			printf("Block Data: ");
			unsigned char *cc = (unsigned char *)work.data;
			for (i=0;i<80;i++) {
				printf(",0x%02X", cc[i]);
			}
			printf("\n");
		...
		addr = UINT64_C(0x504);
		uint32_t chunks = (uint32_t)(g_fpga_work_len / 4);
		for (i = 0; i < chunks; i++) {
		...
		while (mythr && !work_restart[thr_id].restart) {
			...
			for (i = 0; i < 10; i++) {
				...
				if ((value != 0) && (value != old_value)) {
					...
					ret = 1;
//					addr = UINT64_C(0x050);
					applog(LOG_DEBUG, "Nonce Found: %08X", nonce);
					break;
				}
				ret = 0;
				nmsleep(5);
...
//		aes_ni_supported = has_aes_ni();
	...
	switch (opt_algo) {
		...
		case ALGO_VANILLA:
			g_fpga_work_len = 44;
			g_fpga_mid_len = 32;
			g_fpga_data_start = 64;
			g_fpga_data_len = 12; // 80 - 64 Bytes
			g_fpga_use_target = false;
		...
		case ALGO_DECRED:
			g_fpga_work_len = 84;
			g_fpga_mid_len = 32;
			g_fpga_data_start = 128;
			g_fpga_data_len = 52; // 180 - 128
			g_fpga_use_target = false;
			g_fpga_swap_data = false;
			g_fpga_nonce_idx = 35;
			break;
		case ALGO_GROESTL:
		case ALGO_DMD_GR:
		...
		case ALGO_POLY:
		case ALGO_SKUNK:
			g_fpga_work_len = 80;		// Length of message to send to FPGA
			g_fpga_mid_len = 64;		// Length of midstate (if applicable)
			g_fpga_data_start = 64;		// If using midstate, index of data in work packet for remaining data; otherwise set to 0
			g_fpga_data_len = 12;		// Length of remaining block header data to send to FPGA
			g_fpga_use_target = true;
			g_fpga_swap_data = false;
			g_fpga_nonce_idx = 19;
			break;
		case ALGO_TRIBUS:
			g_fpga_work_len = 144;		// Length of message to send to FPGA
			g_fpga_mid_len = 128;		// Length of midstate (if applicable)
			g_fpga_data_start = 64;		// If using midstate, index of data in work packet for remaining data; otherwise set to 0
			g_fpga_data_len = 12;		// Length of remaining block header data to send to FPGA
			g_fpga_use_target = true;
			g_fpga_swap_data = false;
